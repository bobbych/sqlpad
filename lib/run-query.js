...
var snowflake = require('snowflakedb')
...
clients.snowflake = function (query, connection, queryResult, callback) {
  var snowflakeConfig = {
  ...
  var snowflakeConnection = snowflake.createConnection(snowflakeConfig)
  ...
  snowflakeConnection.connect(function (err) {
    ...
    var snowflakeQuery = snowflakeConnection.execute({
          sqlText: query
      }).streamRows()
    ...
    snowflakeQuery
      ...
      .on('data', function (row) {
        ...
        if (rowCounter <= connection.maxRows) {
          ...
          // It sounds like there is no way to close query stream
          // you just have to close the connection
          snowflakeConnection.pause()
          queryResult.incomplete = true
          continueOn() // return records to client before closing connection
          snowflakeConnection.end()
        }
      })
      .on('end', function () {
         // all rows have been received
